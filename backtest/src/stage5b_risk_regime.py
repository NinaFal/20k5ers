#!/usr/bin/env python3
"""
Stage 5b — Fine-grained risk regime search with DDD penalty.

Differences from Stage 5:
  - RISK_VOLATILE_MULT and RISK_CALM_MULT use step=0.01 (was 0.05)
    so the optimizer can find the exact DDD safety boundary.
  - Objective adds a DDD penalty term (same weight as TDD penalty)
    so Optuna actively steers away from DDD-violating configs.
  - Search space for vol narrowed to 0.60-1.30 (boundary region).
  - Seeds from OOS-verified safe high-profit configs (trials 20, 78, 129).
  - New study "stage5b" in stage5b.db (independent of stage5).

Usage:
    python -u backtest/src/stage5b_risk_regime.py [--trials 300] [--jobs 4]
"""
import argparse
import csv
...
# Seeds: OOS-verified safe high-profit configs
SEEDS = [
...
def _cleanup_zombies(db_path: str):
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            conn.commit()
        if n:
            print(f"[stage5b] cleaned {n} zombie RUNNING trial(s) → FAIL", flush=True)
    except Exception as e:
        print(f"[stage5b] zombie cleanup warning: {e}", flush=True)
...
def main():
    ...
    db_path  = str(DOE_DIR / "stage5b.db")
    csv_path = DOE_DIR / "stage5b.csv"

    _cleanup_zombies(db_path)

    study = optuna.create_study(
        direction="maximize", study_name="stage5b",
    ...
    if not non_fail:
        ...
        print(f"[stage5b] Enqueued {len(SEEDS)} seed trials", flush=True)

    done = sum(1 for t in study.trials
               if t.state in (optuna.trial.TrialState.COMPLETE,
                              optuna.trial.TrialState.PRUNED))
    remaining = max(0, args.trials - done)
    print(f"[stage5b] DDD+TDD penalties  step=0.01 for vol/calm  "
    ...
    print(f"\n[stage5b] COMPLETE — {len(survivors)} survivors", flush=True)
    if survivors:
    ...
    print("[stage5b] STAGE5B_DONE_MARKER", flush=True)
