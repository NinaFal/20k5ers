#!/usr/bin/env python3
"""
Stage 5b OOS Screener — flat-pool design.

Submits ALL (top-N trials × 6 OOS windows) = up to 120 tasks to one
ThreadPoolExecutor so workers never idle between trials.

Usage:
    python -u backtest/src/stage5b_oos_screen.py [--top 20] [--workers 6]

Reads  : backtest/output/doe/stage5b.csv
Writes : backtest/output/doe/stage5b_oos_screen.json
         backtest/output/doe/stage5b_oos_screen_report.txt
"""
import argparse
...
CSV_IN     = DOE_DIR / "stage5b.csv"
JSON_OUT   = DOE_DIR / "stage5b_oos_screen.json"
REPORT_OUT = DOE_DIR / "stage5b_oos_screen_report.txt"
...
def write_report(results: list[dict], top_n: int):
    lines = [
        f"Stage 5b OOS Screen — top {top_n} trials  [{ts()}]",
    ...
    if passing:
        ...
        for k, v in best["env"].items():
            lines.append(f"    {k} = {v}")
    lines += ["", "[stage5b_oos_screen] STAGE5B_OOS_SCREEN_DONE_MARKER"]
    text = "\n".join(lines) + "\n"
...
def main():
    ...
    JSON_OUT.write_text(json.dumps(results, indent=2, default=str))
    write_report(results, args.top)
    print("[stage5b_oos_screen] STAGE5B_OOS_SCREEN_DONE_MARKER", flush=True)
