#!/usr/bin/env bash
# watchdog_stage5.sh — supervised launcher for Stage 5 risk regime optimizer.
#
# Launch:
#   setsid nohup bash backtest/src/watchdog_stage5.sh \
#     >> backtest/output/doe/watchdog_stage5.log 2>&1 &

set -euo pipefail
cd "$(git rev-parse --show-toplevel)"

TRIALS="${STAGE5_TRIALS:-150}"
WORKERS="${VAL_WORKERS:-4}"

LOG="backtest/output/doe/stage5_run.log"
...
commit_push() {
    git add -f "$LOG" "$CSV" watchdog_stage5.log 2>/dev/null || true
    if ! git diff --cached --quiet 2>/dev/null; then
        git commit -m "chore: stage5 risk regime checkpoint $(ts)" --quiet 2>/dev/null || true
...
echo "[$(ts)] watchdog_stage5 starting (TRIALS=$TRIALS WORKERS=$WORKERS pid=$$)"

while true; do
    if [ "$(done_marker "$LOG")" -ge 1 ]; then
        commit_push
        echo "[$(ts)] STAGE5_ALL_COMPLETE — watchdog exiting"
        break
    fi
